...
class Pexdex extends CI_Controller {
	...
	public function __construct()
    {
        ...
        $this->load->model(['common',"pexdex_model"]);
    }
    ...
    public function index(){
        ...
        if($this->input->server('REQUEST_METHOD')=='POST')
        {
            ...
            $this->common->save("delapp_deliveries",$save);
                ...
                if(!empty($id)){
                    $order_id = $this->common->record("delapp_deliveries",["id"=>$id]);
                    }
                $this->session->all_userdata($order_id->order_id);
                redirect('pexdex/packageDetail/'. $order_id->order_id);
            }
        }
    ...
    public function packageDetail($id ="")
    {
      ...
      //  if(!empty($id)){
       ...
       // }
        ...
        if(!empty($id)){
            $this->data['package'] = $this->common->record("delapp_deliveries",["order_id"=>$id]);
            }
     ...
     //   $this->data['packages'] = $this->common->records("delivery_package");
        if($this->input->server('REQUEST_METHOD')=='POST'){
            ...
            if(!empty($id)){
            $this->common->update("delapp_deliveries",$save,['order_id'=>$id]);
                //0print_r($save); die;
            redirect('pexdex/service/'. $id);
            }
    ...
    public function weightCal($id=""){
        ...
        if($this->input->server('REQUEST_METHOD')=='POST'){
            ...
            if ($this->form_validation->run()== TRUE)
            {
                   ...
                   // redirect('pexdex/packageDetail/'. $id);
                    }
            }
            else
            {
    ...
    public function service($id=""){
           ...
           // if(!empty($id)){
                $this->data['service'] = $this->pexdex_model->cloonlineData($id);
              //  }
    ...
    public function payment($id){
        ...
        if(!empty($id)){
        $this->data['payment'] = $this->pexdex_model->cloonlineData($id);
        }
    ...
    public function StartPayment($id)
    {
        ...
        if(!empty($id)){
            $this->data['amount'] = $this->common->record("delapp_deliveries",["order_id"=>$id]);
            }
        ...
        $paramList["CUST_ID"] = $payment->id;   /// according to your logic
        $paramList["INDUSTRY_TYPE_ID"] = 'RETIAL';
        $paramList["CHANNEL_ID"] = 'WEB';
        $paramList["TXN_AMOUNT"] = $payment->paid_amount;
        $paramList["WEBSITE"] = PAYTM_MERCHANT_WEBSITE;
        $paramList["CALLBACK_URL"] = "https://chaloonline.in/pexdex/paymentstatus/".$id;
...
<form id="myForm" action="<?php echo PAYTM_TXN_URL ?>" method="post">
 ...
 <?php
            ...
            if ($_POST["STATUS"] == "TXN_SUCCESS") {
              //  echo "<b>Transaction status is success</b>" . "<br/>";
                ...
                if(!empty($id)){
                    $this->common->update("delapp_deliveries",$save,['order_id'=>$id]);
                   // print_r($save);die;
                    redirect('pexdex/PaytmSucessResponse/'. $id);
                }
            }
            else {
              //  echo "<b>Transaction status is failure</b>" . "<br/>";
                ...
                if(!empty($id)){
                    $this->common->update("delapp_deliveries",$save,['order_id'=>$id]);
                    redirect('pexdex/PaytmFailResponse/'. $id);
                }
            }
    ...
    if(!empty($id)){
        $this->data['orderDetail'] = $this->pexdex_model->cloonlineData($id);
        }
    ...
    public function ordertrack(){
        ...
        if ($this->form_validation->run()== TRUE)
        {
            ...
            if(!empty($Num)){
                $orderdata = $this->pexdex_model->orderhistoryData($Num);
    ...
    public function orderHistory()
    {
        ...
        if(!empty($Num)){
            $this->data['orderdata'] = $this->pexdex_model->orderhistoryData($Num);
            }
    ...
    {
        if(!empty($orderId)){
            $this->data['orderdetail'] = $this->pexdex_model->orderdetailData($orderId);
            }
